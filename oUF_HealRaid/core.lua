...
do
    indicatorList = {
        DRUID = {
            ...
            {155777, 'TOPRIGHT', {1, 0.4, 1}},  -- Germination
...
local function CreateIndicators(self, unit)
    ...
    if (buffs) then
        for key, spell in pairs(buffs) do
            ...
            icon.icon:SetTexture('Interface\\AddOns\\oUF_HealRaid\\media\\borderIndicator')
            ...
            if (not icon.hideCount) then
                ...
                icon.count:SetFont('Interface\\AddOns\\oUF_HealRaid\\media\\fontVisitor.ttf', 13)
            end
...
local function CreateRaidLayout(self, unit)
    ...
    self.Health.Value:SetPoint('TOP', self, 'CENTER', 0, -3)
    self.Health.Value:SetFont(config.font.fontSmall, config.font.fontSmallSize)
    self.Health.Value:SetTextColor(1, 0, 0, 0)
   -- self.Health.Value:SetShadowOffset(1, -1)

        -- Name text

    self.Name = self.Health:CreateFontString(nil, 'OVERLAY')
    self.Name:SetPoint('CENTER', self.Health, 'CENTER', 2, 0)
    self.Name:SetFont(config.font.fontBig,config.font.fontBigSize,"OUTLINE")
    --self.Name:SetShadowOffset(1, -1)
    self.Name:SetTextColor(1, 1, 1)
    self:Tag(self.Name, '[name:def]')
    ...
    self.ThreatGlow:SetBackdrop({edgeFile = 'Interface\\AddOns\\oUF_HealRaid\\media\\textureGlow', edgeSize = 3})
    self.ThreatGlow:SetBackdropBorderColor(0, 0, 0, 0)
    ...
    if (config.units.raid.showThreatText) then
        ...
        self.ThreatText:SetFont(config.font.fontSmall, 8, 'THINOUTLINE')
    ...
    if (config.units.raid.showRolePrefix) then
        ...
        self.LFDRoleText:SetPoint('TOPRIGHT', self.Health, 2, -2)
        self.LFDRoleText:SetFont(config.font.fontSmall, 8,"OUTLINE")
    ...
    if (config.units.raid.showTargetBorder) then
        ...
        self.TargetBorder:SetTexture('Interface\\Addons\\oUF_HealRaid\\media\\borderTarget')
...
local f = CreateFrame('Frame', 'oUF_Heal_Raid_Anchor', UIParent)
f:SetSize(80, 80)
f:SetPoint('CENTER')
...
f.t:SetText('oUF_Heal Raid_Anchor "'..config.units.raid.layout.initialAnchor..'"')
...
SlashCmdList['oUF_Neav_Raid_AnchorToggle'] = function()
    if (InCombatLockdown()) then
        f:Hide()
        print('oUF_HealRaid: You cant do this in combat!')
        return
    end
...
oUF:RegisterStyle('oUF_Heal_Raid', CreateRaidLayout)
oUF:Factory(function(self)
    self:SetActiveStyle('oUF_Heal_Raid')
    ...
    for i = 1, rlayout.numGroups do
        table.insert(raid, self:SpawnHeader('oUF_Heal_Raid'..i, nil, 'solo,party,raid',
